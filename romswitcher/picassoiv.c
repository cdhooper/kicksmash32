...
p4_program_rgb_loads(const p4_ff_t *ff)
{
    ...
    /*
     * Picasso IV ROM helpers 0x07f80 and 0x08094 program these board-side
     * controls through control_base + 0x800 + index * 2.
     */
...
{
    /*
     * ROM helper 0x08094 re-applies these two control writes for boards
     * connected to the video slot. Keep this separate from the polled RGB
     * load helper above because the ROM does the late pass separately.
     */
    p4_control_index_write(ff, 0x224, 0x3e);
    p4_control_index_write(ff, 0x225, 0xff);
    p4_control_index_write(ff, 0x224, 0x32);
...
{
    /*
     * ROM helper 0x08102 programs the video-slot colour path after the RGB
     * load handshake succeeds. These are the literal command/data pairs sent
     * via helpers 0x07eea and 0x07f2e.
     */
