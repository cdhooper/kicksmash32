...
static const cmd_t cmd_list[] = {
    ...
    { cmd_power,   "power",   2, cmd_power_help, " [on|off|show]",
                        "show or manage Kicksmash power" },
...
#endif
    { cmd_smash,   "smash",   2, cmd_power_help, " [id|test]",
                        "smash utility commands" },
#ifdef HAVE_SPACE_PROM
...
cmdline(void)
{
    ...
    if (config.name[0] == '\0')
        ...
        sprintf(prompt, "%s CMD> ", config.name);

    (void) get_new_input_line(prompt, &line);
    if (line != NULL) {
        HIST_ENTRY *hist_cur;
