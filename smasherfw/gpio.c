...
gpio_init(void)
{
    ...
    /* Pull up on Amiga reset */
    gpio_setv(KBRST_PORT, KBRST_PIN, 1);
    gpio_setmode(KBRST_PORT, KBRST_PIN, GPIO_SETMODE_INPUT_PULLUPDOWN);

    /* Turn off status LED */
    gpio_setv(LED_STATUS_PORT, LED_STATUS_PIN, 1);
    gpio_setmode(LED_STATUS_PORT, LED_STATUS_PIN, GPIO_SETMODE_OUTPUT_PPULL_2);

    /* Pull down unused pins */
    gpio_setv(GPIOB, GPIO1 | GPIO12, 0);
    gpio_setmode(GPIOB, GPIO1 | GPIO12, GPIO_SETMODE_INPUT_PULLUPDOWN);

    /* Pull up SOCKET_OE (output enable) */
    ...
    gpio_setmode(SOCKET_CE_PORT, SOCKET_CE_PIN, GPIO_SETMODE_OUTPUT_PPULL_50);

    /* Give socket A18 and A19 weak pull down */
    gpio_setv(SOCKET_A18B_PORT, SOCKET_A18B_PIN | SOCKET_A19B_PIN, 0);
    gpio_setmode(SOCKET_A18B_PORT, SOCKET_A18B_PIN | SOCKET_A19B_PIN,
                 GPIO_SETMODE_INPUT_PULLUPDOWN);

    /* Give D0-D31 weak pull down */
    gpio_setv(SOCKET_D0_PORT,  0xffff, 0);
    gpio_setv(SOCKET_D16_PORT, 0xffff, 0);
