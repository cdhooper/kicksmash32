...
"set mode <num>             - set prom mode (0=32 1=16 2=16hi 3=auto 4=32swp)\n"
"set name <name>            - set Kicksmash board name";

const char cmd_power_help[] =
"power cycle - cycle power to Kicksmash board\n"
"power on    - turn on power to Kicksmash board\n"
"power off   - turn off power to Kicksmash board\n"
...
cmd_prom(int argc, char * const *argv)
{
    ...
    switch (op_mode) {
        ...
        case OP_ERASE_SECTOR:
            ...
            if (len == 0)
                len = 0xffffffff; // Value unassigned
...
static const config_set_t config_set[] = {
    ...
    { "leaveon",        "Leave Kicksmash powered",
      CFOFF(leave_on), MODE_DEC },
};
...
cmd_set(int argc, char * const *argv)
{
    ...
    if (strcmp(argv[1], "bank") == 0) {
        ...
        if (do_save)
            config_updated();
    } else if (strcmp(argv[1], "leaveon") == 0) {
        if (argc <= 2) {
            printf("leaveon %u\n", config.leave_on);
            return (RC_SUCCESS);
        }
