...
static int
warn_amiga_in_reset(void)
{
    if (!amiga_not_in_reset) {
        printf("Fail: Kicksmash is driving KBRST\n");
        return (1);
    }
    return (0);
}
...
prom_read(uint32_t addr, uint width, void *bufp)
{
    ...
    uint bank = addr >> 19;  // 0x80000
    rc_t rc;
    addr &= 0x7ffff;
    ee_enable();
    rc = read_from_flash(bank, addr, bufp, width);
    if ((rc == 0) || (bank != 0))
        return (rc);
    /* Fall back to direct read */
    if (warned == 0) {
        warned = 1;
        printf("Kicksmash bank select failed: %d -- using direct read\n", rc);
    }

    /* Handle 32-bit mode separate from 16-bit modes */
    if ((ee_mode == EE_MODE_32) || (ee_mode == EE_MODE_32_SWAP))
        return (prom_read_32(addr, width, bufp));
    else
        return (prom_read_16(addr, width, bufp));
    return (0);
}
...
prom_write(uint32_t addr, uint width, void *bufp)
{
    ...
    addr &= 0x7ffff;
    ee_enable();
    return (write_to_flash(bank, addr, bufp, width));
}
...
prom_erase(uint mode, uint32_t addr, uint32_t len)
{
    ...
    addr &= 0x7ffff;
    ee_enable();
    return (erase_flash(bank, addr, len, 1));
}
