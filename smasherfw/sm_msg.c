...
#define STATIC_ASSERT(e, n) enum { \
            ASSERT_CONCAT(ASSERT_CONCAT(ASSERT_CONCAT(assert_line_, __LINE__), \
            ASSERT_CONCAT(_, __COUNTER__)), n) = 1/(!!(e)) \
        }
#define STATIC_ASSERT_SIZE(type, size) STATIC_ASSERT(sizeof (type) == (size), type)
#elif 1
...
{
    return (send_cmd_core(cmd, arg, arglen, reply, replymax, replyalen));
}
...
host_send_msg(void *smsg, uint len)
{
    ...
    if ((rc == 0) && (sendlen < len)) {
        ...
        while (pos < len - sizeof (km_msg_hdr_t)) {
            ...
            memcpy(savebuf, (uint8_t *)smsg + pos, sizeof (km_msg_hdr_t));
            memcpy((uint8_t *)smsg + pos, smsg, sizeof (km_msg_hdr_t));

#undef DEBUG_SEND_MSG
#ifdef DEBUG_SEND_MSG
            printf("send %x pos=%x of %x\n", sendlen, pos, len);
#endif
            rc = send_cmd_retry(KS_CMD_MSG_SEND, (uint8_t *)smsg + pos, sendlen,
                                NULL, 0, NULL);
            memcpy((uint8_t *)smsg + pos, savebuf, sizeof (km_msg_hdr_t));
...
host_recv_msg_cont(uint tag, void *buf, uint buf_len)
{
    ...
    while (cur_len < buf_len) {
        ...
        memcpy((uint8_t *)buf + cur_len, rdata + sizeof (km_msg_hdr_t), rcvlen);
        cur_len += rcvlen;
    }
    return (KM_STATUS_OK);
}
...
STATIC_ASSERT(ARRAY_SIZE(ks_status_s) == (KS_STATUS_LAST_ENT >> 8), ks_status_t);
...
STATIC_ASSERT(ARRAY_SIZE(km_status_s) == KM_STATUS_LAST_ENTRY, km_status_t);
...
STATIC_ASSERT(ARRAY_SIZE(msg_status_s) ==
              (MSG_STATUS_FAIL - MSG_STATUS_LAST_ENTRY), msg_status_t);
